...
class MetabolicWorld(RelationalWorld):
    ...
    def execute(self, cmd):
        ...
        active, paid = job.plan.path[0], job.completed_units
        for node, units in zip(job.plan.path[1:], job.plan.hop_units):
            if paid < units: break
            active, paid = node, paid - units
