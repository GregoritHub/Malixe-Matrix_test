...
class OrganizationWorld(LanguageWorld):
    def __init__(self,config,profiles,policy,agents,organization_policies=None):
    ...
    def execute(self,cmd):
        if type(cmd) is not OrganizationCommand: return super().execute(cmd)
        prior=self._commands.get(cmd.command_id)
        if prior is not None:
            if prior.command!=cmd: raise ValueError('command ID reused')
            return prior.event
        old=self.organization_job(cmd.actor,cmd.task_id)
        if old is not None and (old.command.payload!=cmd.payload or old.outcome in (WorkStatus.COMPLETED,WorkStatus.FAILED)):
            raise ValueError('changed or terminal organization job')
        required,r=self._prepare_organization(cmd)
        if old is None: old=OrganizationJob(cmd,required,r)
        wallet=self._wallets[cmd.actor]
        spent=min(old.required-old.paid,cmd.work_limit,wallet.energy,wallet.time)
        paid=old.paid+spent
        status=WorkStatus.COMPLETED if paid==old.required else WorkStatus.PARTIAL if paid else WorkStatus.DEFERRED
        if required!=old.required or r!=old.candidate: status=WorkStatus.FAILED
        extra=(r,) if status==WorkStatus.COMPLETED else ()
        job=replace(old,paid=paid,outcome=status,result=r.ref if extra else None)
        when=Moment(len(self._journal),0)
        work=WorkRecord(Ref(Kind.WORK,f'work:{when.tick}:0',1),cmd.actor,ORGANIZE,amounts(wallet),(),
            (ResourceAmount(ENERGY,spent),ResourceAmount(TIME,spent)),
            amounts(Wallet(cmd.actor,wallet.energy-spent,wallet.time-spent)),old.required-old.paid,spent,status,
            'paid local organization operation; simulation validity is separate')
        causes=tuple(dict.fromkeys(Cause(x if x.kind==Kind.EVENT else self._origins[x],ORGANIZATION) for x in r.sources))
        event=WorldEvent(Ref(Kind.EVENT,f'event:{when.tick}',1),when,(cmd.actor,),(),'r9.'+type(cmd.payload).__name__,
            self.config.context,(),causes,(work.ref,),status,'owned procedural selection or exact local consent')
        self._commit(OrganizationTransaction(cmd,event,(work,),extra=extra,job=job))
        return event
    ...
    def _commit(self,tx):
        ...
        if type(tx) is OrganizationTransaction: self._organization_jobs[(tx.command.actor,tx.command.task_id)]=tx.job
    ...
    def checkpoint(self):
        ...
        return dumps(OrganizationCheckpoint('hle-r9-v1',self.config,self.profiles,self.policy,self.agents,
            self.organization_policies,tuple(self._journal)))

    @classmethod
    def restore(cls,text):
        from .codec import loads
        cp=loads(text)
        if type(cp) is not OrganizationCheckpoint or cp.schema!='hle-r9-v1' or not cp.journal:
            raise ValueError('unsupported or empty organization checkpoint')
        w=cls(cp.config,cp.profiles,cp.policy,cp.agents,cp.organization_policies)
