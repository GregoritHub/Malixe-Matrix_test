...
class LanguageWorld(ComposedWorld):
    ...
    def execute(self,cmd):
        if type(cmd) is not LanguageCommand: return super().execute(cmd)
        prior=self._commands.get(cmd.command_id)
        if prior is not None:
            if prior.command!=cmd: raise ValueError('command ID reused')
            return prior.event
        old=self.language_job(cmd.actor,cmd.task_id)
        if old is not None and (old.command.payload!=cmd.payload or old.outcome in (WorkStatus.COMPLETED,WorkStatus.FAILED)):
            raise ValueError('changed or terminal language job')
        when=Moment(len(self._journal),0)
        required,result,basis=self._prepare(cmd,when)
        # At each funded retry, revalidate current evidence and definitions. A changed
        # required extent is a paid failure, never silent reuse of old partial credit.
        if old is None: old=LanguageJob(cmd,required,result)
        wallet=self._wallets[cmd.actor]; spent=min(old.required-old.paid,cmd.work_limit,wallet.energy,wallet.time)
        paid=old.paid+spent
        status=WorkStatus.COMPLETED if paid==old.required else WorkStatus.PARTIAL if paid else WorkStatus.DEFERRED
        if required!=old.required or result!=old.candidate: status=WorkStatus.FAILED
        ...
        work=WorkRecord(Ref(Kind.WORK,f'work:{when.tick}:0',1),cmd.actor,LANGUAGE_WORK,amounts(wallet),(),
            ...
            'paid semantic work; endpoint support is separate from factual accuracy')
        causes=tuple(dict.fromkeys(Cause(self._origins[r],LANGUAGE) for r in basis))
        event=WorldEvent(Ref(Kind.EVENT,f'event:{when.tick}',1),when,(cmd.actor,),(),'r8.language',
            self.config.context,(),causes,(work.ref,),status,'retained semantic operation')
        self._commit(LanguageTransaction(cmd,event,(work,),extra=extra,job=job))
        return event
    ...
    def _commit(self,tx):
        ...
        if type(tx) is LanguageTransaction: self._language_jobs[(tx.command.actor,tx.command.task_id)]=tx.job
    ...
    def checkpoint(self):
        from .codec import dumps
        return dumps(LanguageCheckpoint('hle-r8-v1',self.config,self.profiles,self.policy,self.agents,tuple(self._journal)))

    @classmethod
    def restore(cls,text):
        from .codec import loads
        cp=loads(text)
        if type(cp) is not LanguageCheckpoint or cp.schema!='hle-r8-v1' or not cp.journal:
